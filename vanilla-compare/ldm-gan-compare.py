...
"""
This script runs a focused experiment to compare FOUR balancing strategies:
1.  Orig_Imbalanced: Real, imbalanced data (168H, 241C)
2.  Real_Balanced_Undersample: Real, undersampled data (168H, 168C)
3.  Synthetic_Balanced (LDM): Real data + LDM-generated balancing data (241H, 241C) - DYNAMIC GENERATION
4.  cGAN_Balanced: Real data + cGAN-generated balancing data (241H, 241C) - LOADED WEIGHTS

This will isolate the effect of different balancing techniques.
"""
...
# Output Directory
OUT_DIR = Path(r"../Balancing_Experiment_Results_with_cGAN_Dynamic")
...
# =================================================================
# PART 2: Generative model definitions (LDM & AE)
# =================================================================
def gnorm(c):
    return nn.GroupNorm(num_groups=min(4, c), num_channels=c)


# --- CORRECTED ConvAE CLASS ---
class ConvAE(nn.Module):
    def __init__(self, F: int, downs: int = 4, base: int = 64, latent_c: int = 12):
        ...
        # --- FIX APPLIED HERE: RESTORED THE PROBE BLOCK ---
        with torch.no_grad():
...
# =================================================================
# PART 3: LDM GENERATOR FUNCTION (SAMPLER)
# =================================================================
@torch.no_grad()
def generate_clean_spectra_ldm(unet, ae, z_mu, z_std, z_tr_std, T_trained, steps, y_class, n, w):
    ...
    # THIS LINE WILL NOW WORK because ae.latent_L is defined in __init__
...
# =================================================================
# PART 4: cGAN MODEL DEFINITION & SAMPLER
# =================================================================
...
# =================================================================
def main():
    print("Starting Balancing Strategies Comparison (XGBoost) - including cGAN...")
    ...
    # --- 7. Final Report ---
    print("\n" + "=" * 60)
    print("     BALANCING STRATEGY COMPARISON (XGBOOST) - FINAL")
    print("=" * 60)

    df_results = pd.DataFrame(all_results)
    df_results.set_index("strategy", inplace=True)

    results_csv_path = OUT_DIR / "balancing_strategy_comparison_with_cgan.csv"
    ...
    colors = ['tab:blue', 'tab:orange', 'tab:green', 'tab:red']  # Different color for cGAN

    for i, metric in enumerate(['test_auc', 'test_acc', 'test_sens', 'test_spec']):
        row = i // 2
        col = i % 2
        ax = axes[row, col]
        # Use a distinct color for the cGAN bar if it exists
        bar_colors = [colors[0]] * len(strategies_str)
        if "cGAN_Balanced" in strategies_str:
            cgan_idx = np.where(strategies_str == "cGAN_Balanced")[0][0]
            bar_colors[cgan_idx] = colors[3]

        # Highlight LDM as well
        if "Synthetic_Balanced (LDM)" in strategies_str:
            ldm_idx = np.where(strategies_str == "Synthetic_Balanced (LDM)")[0][0]
            bar_colors[ldm_idx] = colors[2]
    ...
    plot_path = OUT_DIR / "balancing_strategy_comparison_with_cgan.png"
