...
def timestep_embedding(timesteps, embed_dim):
    ...
    freqs = torch.exp(
        -np.log(10000) * torch.arange(0, half_dim, dtype=torch.float32, device=timesteps.device) / half_dim)
...
def p_sample_ddpm(model, x_t, t, betas, alpha_bars, device, cond, guidance_scale=5.0):
    ...
    x0_pred = (x_t - sqrt_one_minus_alpha_bar_t * pred_noise) / torch.sqrt(alpha_bars[t - 1])
...
# -------------------------------
def create_peak_mask(length=234,  # Changed to 234
                     start_wn=1797.53,  # Changed to 1801.39
                     end_wn=898.764,  # Changed to 898.764
...
def peak_weighted_mse(x_pred, x_true, peak_mask):
    ...
    # Expand peak_mask to match shape (batch,1,length)
    # We'll do broadcast by (1,1,length)
    w = torch.from_numpy(peak_mask).to(x_pred.device).view(1, 1, -1)
    diff2 = (x_pred - x_true) ** 2
    return (diff2 * w).mean()

# -------------------------------
# 6. Contrastive Net with Triplet Loss
# -------------------------------
...
def get_triplets(embeddings, labels, margin=1.0):
    """Generate hard triplets for batch-hard triplet loss"""
    n = embeddings.size(0)
    pairwise_dist = torch.cdist(embeddings, embeddings)

    triplets = []
    for i in range(n):
        # Mask for positive samples (same class, not self)
        arange_tensor = torch.arange(n, device=labels.device)
        pos_mask = (labels == labels[i]) & (arange_tensor != i)

        pos_indices = pos_mask.nonzero().squeeze(1)

        # Mask for negative samples (different class)
        ...
        hardest_pos = pairwise_dist[i][pos_indices].argmax()

        # Hardest negative (closest)
        hardest_neg = pairwise_dist[i][neg_indices].argmin()

        triplets.append((i, pos_indices[hardest_pos], neg_indices[hardest_neg]))

    return torch.LongTensor(triplets) if triplets else None


def contrastive_step(contrastive_net, batch_x, batch_y, device, margin=1.0):
    """Compute triplet loss for a batch"""
    batch_x = batch_x.to(device)
    batch_y = batch_y.to(device)

    # Get embeddings
    embeddings = contrastive_net(batch_x)

    # Generate triplets
    triplets = get_triplets(embeddings, batch_y)
    if triplets is None:
        return None
    ...
    return F.triplet_margin_loss(anchors, positives, negatives, margin=margin)
...
def plot_single_class(real_data, synth_data, class_label, length=234,save_dir="results"):
    ...
    plt.plot(wave_numbers, synth_data, label="Synthetic Spectrum",
             linestyle="--", linewidth=1.5, alpha=0.8)

    # Add vertical lines for important peaks
    peak_positions = [1446.51, 1377.08, 1234.35, 1045.34, 902.622]
    for wn in peak_positions:
        plt.axvline(x=wn, color='gray', linestyle=':', alpha=0.5,
                    label='Peak Region' if wn == peak_positions[0] else None)
    ...
    plt.close()  # Important for memory management on HPC

    print(f"Saved plot for class {class_label} to {save_dir}/")
    #plt.show()


def verify_peak_regions(real, synth, wavenumbers, peak_positions, window_size=10):
    """Statistical comparison of peak regions"""
    ...
    for wn in peak_positions:
        ...
        area_diff = abs(real_area - synth_area) / real_area * 100  # % difference
...
def main():
    ...
    X_scaled, y, _ = load_and_preprocess_data("train_set.csv")
    # Reshape to (N,1,length)
    X_scaled = X_scaled[:, None, :]
    length = X_scaled.shape[2]
    ...
    # Weighted Loss Hyperparams was 2
    lambda_signal = 6
    lambda_peaks = 2
    ...
    print(f"Peak mask weights: {np.unique(peak_mask)}")
    ...
    for epoch in range(epochs):
        ...
        print(f"Component losses - Noise: {np.mean(noise_losses):.4f}, "
                  f"Signal: {np.mean(signal_losses):.4f}, Peaks: {np.mean(peak_losses):.4f}")
    ...
    triplet_margin = 1.0  # Can adjust based on your data
    print("Training Contrastive Network with Triplet Loss...")
    ...
    for epoch in range(10):
        ...
        for batch_x, batch_y in dataloader:
            # Skip batches with only one class
            if len(torch.unique(batch_y)) < 2:
                continue

            loss = contrastive_step(contrastive_net, batch_x, batch_y, device, triplet_margin)
            if loss is None:
                continue
        ...
        if cl_losses:
            avg_loss = np.mean(cl_losses)
            print(f"Contrastive Epoch {epoch + 1}/10 - Loss: {avg_loss:.6f} (Valid batches: {valid_batches})")
        else:
            print(f"Contrastive Epoch {epoch + 1}/10 - No valid triplets in any batch")

    # Evaluate
    model.eval()
    contrastive_net.eval()

    # Dictionary to store results for all classes
    class_results = {}
    ...
    # Plot and analyze each class separately
    for cl in class_results:
        ...
        verify_peak_regions(data['real'], data['synth'],
                            np.linspace(1797.53, 898.764, length),
                            [1446.51, 1377.08, 1234.35, 1045.34, 902.622])
