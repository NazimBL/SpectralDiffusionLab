...
# drop Hyperplasia (class == 4)
for df in (train_df, val_df, test_df):
    df.drop(df[df['class'] == 4].index, inplace=True)
...
def to_binary(label):
    if label == 0:
        ...
        # should never happen now that 4’s are dropped
        raise ValueError(f"Unexpected label {label}")

for df in (train_df, val_df, test_df):
    df['binary_class'] = df['class'].apply(to_binary)
