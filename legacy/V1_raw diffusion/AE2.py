...
def main():
    ...
    std_true = X_bc.std(axis=0)
    std_dec = dec_bc.std(axis=0)

    # Plotting mean ± std
    plt.figure(figsize=(10, 6))

    # Real spectra
    plt.plot(mean_true, label="Mean Real (baseline-corrected)", color='blue', linestyle='--', lw=2)
    plt.fill_between(np.arange(len(mean_true)),
                     mean_true - std_true,
                     mean_true + std_true,
                     color='blue', alpha=0.2, label='Real ±1 SD')

    # Decoded synthetic spectra
    plt.plot(mean_dec, label="Mean Decoded Synthetic", color='orange', lw=2)
    plt.fill_between(np.arange(len(mean_dec)),
                     mean_dec - std_dec,
                     mean_dec + std_dec,
                     color='orange', alpha=0.2, label='Synthetic ±1 SD')

    plt.xlabel("Wavenumber Index")
    plt.ylabel("Absorbance")
    plt.title("Real vs Synthetic: Mean ± Standard Deviation")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("real_vs_synthetic_std.png", dpi=300)
    plt.show()
