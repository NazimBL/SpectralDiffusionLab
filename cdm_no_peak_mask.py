...
def main():
    ...
    print(f"Loss weights - Signal: {lambda_signal}")
    print(f"Guidance scale: {guidance_scale}")
    
    print("Training 1D U-Net Diffusion Model...")
    model.train()
    for epoch in range(epochs):
        ...
        for batch_x, batch_y in dataloader:
            ...
            loss_total = loss_noise + lambda_signal * loss_signal
        ...
        print(f"Component losses - Noise: {np.mean(noise_losses):.4f}, Signal: {np.mean(signal_losses):.4f}")
    ...
    for cl in class_results:
        ...
        plot_single_class(data['real'], data['synth'], cl, length=length, save_dir="no mask_results")
