...
"""
This script tests a "Pure Latent" + "Balance-then-Augment" strategy
using a simple LOGISTIC REGRESSION classifier.

1.  Real Data is preprocessed: Raw -> Scale -> Encode -> Normalize -> Flatten
2.  Synthetic Data is generated: DDPM -> Rescale -> Flatten
3.  First, the dataset is balanced by generating minority class latents.
4.  Then, increasing ratios (20%-200%) of new, balanced latents are added.
5.  The classifier is trained purely on these (N, 960) latent vectors.
"""
...
AUGMENT_RATIOS = [0.0, 0.4, 0.6, 0.8,1, 1.5, 2]
...
DDPM_CHECKPOINT_FILE = Path("ldm_out/ddpm_latent_unet.pt")
AE_META_FILE = LDM_DIR / "ae_meta.json"
OUT_DIR = Path(r"Augmentation_Results_Pure_Latent_Logit")  # New output folder
SAMPLE_STEPS = 1000
SEED = 42
...
def p_sample_loop_latent(unet, T, steps, y_class, n,
    ...
    """
    if n == 0:
        return np.array([], dtype=np.float32).reshape(0, latent_c, latent_L)
    ...
    # === RETURN THE NORMALIZED LATENT ===
    return z_t.cpu().numpy()  # (N, C, L)
...
def preprocess_with_encoder_and_normalize(X_raw_np: np.ndarray, ae_model: ConvAE,
    ...
    """
    Preprocessing function for REAL data.
    ...
    # 3. Normalize the latents (to match DDPM training)
...
def run_lr_analysis(X_tr: np.ndarray, y_tr: np.ndarray,
    ...
    # C=1.0 is a reasonable default. We could also tune C, but this is simpler.
...
def main():
    global F_LEN
    print("Starting 'Pure Latent' + 'Balance-then-Augment' experiment (with Logistic Regression)...")
    ...
    # --- 3. Preprocess Test Data (Done once using Encoder) ---
    ...
    # --- 4. Run ORIGINAL IMbalanced Baseline (in Latent Space) ---
    print("\n" + "=" * 50)
    ...
    print(f"  Training data preprocessed to shape: {X_tr_orig_preproc.shape}")

    results = run_lr_analysis( # <-- UPDATED
        X_tr_orig_preproc, y_tr_orig,
        X_te_preproc, y_te,
        "Orig_Imbalanced", 0, 0
    )
    all_results.append(results)

    # --- 5. Create the new "Base Balanced Set" (in Latent Space) ---
    print("\n" + "=" * 50)
    print("Creating Base Balanced Dataset (in Latent Space)")
    print("=" * 50)
    print(f"  Generating {n_to_balance} 'Healthy' normalized latents...")
    z_gen_balance_h = p_sample_loop_latent(
        unet=unet, T=T_trained, steps=min(SAMPLE_STEPS, T_trained),
        y_class=0, n=n_to_balance,
        latent_c=latent_c, latent_L=latent_L
    )
    y_gen_balance_h = np.zeros(n_to_balance, dtype=int)
    z_gen_balance_h_flat = z_gen_balance_h.reshape(n_to_balance, -1)

    # This is our new "base" training set of preprocessed latents
    X_tr_base_preproc = np.vstack([X_tr_orig_preproc, z_gen_balance_h_flat])
    ...
    # --- 6. Run Experiment Loop (starting from balanced latent set) ---
    for ratio in AUGMENT_RATIOS:
        ...
        if ratio == 0.0:
            ...
            X_tr_aug_preproc = X_tr_base_preproc
            y_tr_aug = y_tr_base
            ...
            print(f"  Generating {n_syn_per_class} new healthy and {n_syn_per_class} new cancer latents...")

            z_gen_healthy = p_sample_loop_latent(
                unet=unet, T=T_trained, steps=min(SAMPLE_STEPS, T_trained),
                y_class=0, n=n_syn_per_class,
                latent_c=latent_c, latent_L=latent_L
            )
            y_gen_healthy = np.zeros(n_syn_per_class, dtype=int)
            z_gen_healthy_flat = z_gen_healthy.reshape(n_syn_per_class, -1)

            z_gen_cancer = p_sample_loop_latent(
                unet=unet, T=T_trained, steps=min(SAMPLE_STEPS, T_trained),
                y_class=1, n=n_syn_per_class,
                latent_c=latent_c, latent_L=latent_L
            )
            y_gen_cancer = np.ones(n_syn_per_class, dtype=int)
            z_gen_cancer_flat = z_gen_cancer.reshape(n_syn_per_class, -1)

            # Add to the BASE balanced set
            X_tr_aug_preproc = np.vstack([X_tr_base_preproc, z_gen_healthy_flat, z_gen_cancer_flat])
    ...
    # --- 7. Final Report ---
    print("\n" + "=" * 60)
    print("     PURE LATENT 'BALANCE-THEN-AUGMENT' EXPERIMENT (LOGISTIC REGRESSION)")
    print("=" * 60)

    df_results = pd.DataFrame(all_results)
    df_results.set_index("strategy", inplace=True)

    results_csv_path = OUT_DIR / "augmentation_results_pure_latent_logit.csv"
    df_results.to_csv(results_csv_path)
    print(f"Saved results table to: {results_csv_path}")

    print("\nTest Set Performance vs. Augmentation Strategy (Pure Latent, Logistic Regression):")
    # Removed 'best_lv' and 'cv_auc' from the printout
    ...
    fig.suptitle("Logistic Regression Test Performance: Balance-then-Augment Strategy (Pure Latent)", fontsize=16)
    ...
    plot_path = OUT_DIR / "augmentation_metrics_plot_pure_latent_logit.png"
