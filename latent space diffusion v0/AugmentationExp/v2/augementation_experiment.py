...
CLASSIFYING IN THE LATENT SPACE.

1.  Generates synthetic raw spectra.
2.  Combines real + synthetic raw spectra.
3.  Uses the AE ENCODER as the preprocessing/feature extraction step.
4.  Trains and tests the PLS-DA classifier on these latent features.
"""
...
OUT_DIR = Path(r"Augmentation_Results_Latent")  # New output folder
SAMPLE_STEPS = 1000
SEED = 42
ENCODER_BATCH_SIZE = 128  # Batch size for passing data through the encoder
# ==================================
...
def tune_pls_da_auc(X: np.ndarray, y: np.ndarray, k: int, max_lv: int = 30):
    ...
    # Ensure PLS-DA has enough samples in the smallest class for CV
    ...
    for ncomp in range(1, upper + 1):
        ...
        for tr_idx, va_idx in folds:
            ...
            try:
                ...
                continue  # Skip if PLS fails (e.g., not enough samples)
...
def p_sample_loop(unet, ae, T, steps, y_class, n,
    ...
    # (Note: Renamed inputs to avoid collision with global names)
...
def preprocess_with_encoder(X_raw_np: np.ndarray, ae_model: ConvAE,
                            scaler_mean: np.ndarray, scaler_std: np.ndarray,
                            batch_size: int) -> np.ndarray:
    """
    NEW preprocessing function.
    Takes raw numpy spectra, scales them, and runs them through the
    AE encoder, returning flattened latent vectors.
    """
    ae_model.eval()
    ...
    z_flat = z_full.numpy().reshape(z_full.shape[0], -1)  # (N, C*L)
...
def run_pls_analysis(X_tr: np.ndarray, y_tr: np.ndarray,
    ...
    (Slightly modified from previous script)
    """
...
def main():
    print("Starting LATENT SPACE augmentation experiment...")
    ...
    # Tensors for Generator
    x_mean_t = torch.from_numpy(scaler_mean_np).to(DEVICE).view(1, 1, -1)
    x_std_t = torch.from_numpy(scaler_std_np).to(DEVICE).view(1, 1, -1)
    ...
    # --- 3. Preprocess Test Data (Done once using NEW method) ---
    print("Preprocessing test data (encoding to latent space)...")
    X_te_preproc = preprocess_with_encoder(X_te_raw, ae, scaler_mean_np, scaler_std_np, ENCODER_BATCH_SIZE)
    ...
    for ratio in AUGMENT_RATIOS:
        ...
        print("  Preprocessing augmented training data (encoding to latent space)...")
        X_tr_aug_preproc = preprocess_with_encoder(X_tr_aug_raw, ae, scaler_mean_np, scaler_std_np, ENCODER_BATCH_SIZE)
    ...
    # --- 5. Final Report ---
    print("\n" + "=" * 60)
    print("     LATENT SPACE AUGMENTATION EXPERIMENT SUMMARY")
    print("=" * 60)

    df_results = pd.DataFrame(all_results)
    df_results.set_index("ratio", inplace=True)

    results_csv_path = OUT_DIR / "augmentation_results_latent_space.csv"
    df_results.to_csv(results_csv_path)
    print(f"Saved results table to: {results_csv_path}")

    print("\nTest Set Performance vs. Augmentation Ratio (in Latent Space):")
    ...
    fig.suptitle("PLS-DA Test Performance vs. Augmentation Ratio (in Latent Space)", fontsize=16)
    ...
    plot_path = OUT_DIR / "augmentation_metrics_plot_latent_space.png"
