...
"""
This script tests data augmentation using CLASS BALANCING.
It uses the Savgol-filter preprocessing (our best classifier).
"""
...
DDPM_CHECKPOINT_FILE = LDM_DIR / "ddpm_latent_unet.pt"  # The one trained with LAMBDA_PEAKS = 0.05
AE_META_FILE = LDM_DIR / "ae_meta.json"
OUT_DIR = Path(r"Augmentation_Results_Balancing")  # New output folder
...
def tune_pls_da_auc(X: np.ndarray, y: np.ndarray, k: int, max_lv: int = 30):
    ...
    for ncomp in range(1, upper + 1):
        ...
        for tr_idx, va_idx in folds:
            ...
            n_eff = max(1, min(ncomp, min(X[tr_idx].shape[0] - 1, X[tr_idx].shape[1])))
...
def p_sample_loop(unet, ae, T, steps, y_class, n,
    ...
    if n == 0:
        return np.array([], dtype=np.float32).reshape(0, F_LEN)  # Return empty array if 0 samples
...
def run_pls_analysis(X_tr: np.ndarray, y_tr: np.ndarray,
    ...
    print(
        f"  Test Results (Strategy '{strategy_name}'): LV={best_lv}, AUC={auc:.4f}, Acc={acc:.4f}, Sens={sens:.4f}, Spec={spec:.4f}")
...
def main():
    global F_LEN
    print("Starting Savgol Filter BALANCING experiment...")
    ...
    with open(AE_META_FILE, "r") as f:
        meta = json.load(f)
    spec_cols = meta["cols"]
    F_LEN = int(meta["F"])  # Set global F_LEN
    ...
    # Strategies are defined as (n_healthy_to_gen, n_cancer_to_gen)
    STRATEGIES = {
        "Baseline (0%)": (0, 0),
        "Ratio_100%": (n_orig_healthy, n_orig_cancer),
        "Balance_Orig": (n_to_balance, 0),
        "Balance_Large": (n_orig_cancer, n_orig_healthy)  # Makes both classes = n_orig_cancer + n_orig_healthy
    }

    # Let's define a clearer "Balance_Large"
    # We want a large, balanced dataset, e.g., 2x the size
    # New target size for each class = n_orig_cancer * 2 = 482
    n_h_needed_large = (n_orig_cancer * 2) - n_orig_healthy  # 482 - 168 = 314
    n_c_needed_large = n_orig_cancer  # 241

    STRATEGIES = {
        "Baseline": (0, 0),
        "Ratio_100": (n_orig_healthy, n_orig_cancer),
        "Balance_Orig": (n_to_balance, 0),
        "Balance_Large": (n_h_needed_large, n_c_needed_large)
    }
    print("Testing Strategies (n_gen_healthy, n_gen_cancer):")
    for name, (n_h, n_c) in STRATEGIES.items():
        print(f"  - {name}: ({n_h}, {n_c}) -> New Total: {n_orig_healthy + n_h} H, {n_orig_cancer + n_c} C")

    # --- 3. Load Generative Models & Scalers ---
    ...
    # --- 4. Preprocess Test Data (Done once using Savgol) ---
    ...
    for strategy_name, (n_healthy_gen, n_cancer_gen) in STRATEGIES.items():
        print("\n" + "=" * 50)
        ...
        results = run_pls_analysis(
            X_tr_aug_preproc, y_tr_aug,
            X_te_preproc, y_te,
            strategy_name, n_healthy_gen, n_cancer_gen
        )
        all_results.append(results)

    # --- 6. Final Report ---
    print("\n" + "=" * 60)
    print("     SAVGOL FILTER BALANCING EXPERIMENT SUMMARY")
    print("=" * 60)

    df_results = pd.DataFrame(all_results)
    df_results.set_index("strategy", inplace=True)

    results_csv_path = OUT_DIR / "augmentation_results_balancing_savgol_L05.csv"
    df_results.to_csv(results_csv_path)
    print(f"Saved results table to: {results_csv_path}")

    print("\nTest Set Performance vs. Augmentation Strategy (Savgol Filter):")
    print(df_results[['n_total_train', 'n_syn_healthy', 'n_syn_cancer', 'best_lv', 'test_auc', 'test_acc', 'test_sens',
                      'test_spec']].to_string(float_format="%.4f"))

    # --- Plot key metrics ---
    fig, axes = plt.subplots(nrows=2, ncols=2, figsize=(14, 10))
    fig.suptitle("PLS-DA Test Performance vs. Augmentation Strategy (Savgol Filter, Lambda=0.05)", fontsize=16)

    strategies_str = df_results.index.values

    axes[0, 0].bar(strategies_str, df_results['test_auc'])
    ...
    axes[1, 1].bar(strategies_str, df_results['test_spec'], color='tab:blue')
    ...
    plot_path = OUT_DIR / "augmentation_metrics_plot_balancing_savgol_L05.png"
