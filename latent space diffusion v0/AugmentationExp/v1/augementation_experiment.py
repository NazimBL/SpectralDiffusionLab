...
"""
This script runs a full data augmentation experiment.
It combines the PLS-DA classifier from Baseline2.py with the
generative models (AE + DDPM) to test the effect of adding
synthetic data at different ratios.
"""
...
# --- Augmentation Ratios ---
# 0.0 = Baseline (no augmentation)
# 1.0 = 100% (Original data + 100% synthetic data)
...
OUT_DIR = Path(r"Augmentation_Results")

# --- Model Config ---
SAMPLE_STEPS = 1000  # DDPM sampling steps
SEED = 42
# ==================================

# Setup
warnings.filterwarnings('ignore', category=UserWarning)  # Suppress PLS warnings
...
# =================================================================
# PART 1: PLS-DA CLASSIFIER CODE (from Baseline2.py)
# =================================================================
...
def preprocess_numpy_array(X_raw: np.ndarray) -> np.ndarray:
    """
    Applies the baseline preprocessing (SG 2nd deriv + L2 norm)
    to a raw numpy array of spectra.
    """
...
def tune_pls_da_auc(X: np.ndarray, y: np.ndarray, k: int, max_lv: int = 30):
    ...
    for ncomp in range(1, upper + 1):
        ...
        for tr_idx, va_idx in folds:
            ...
            # Ensure n_components is valid for this fold
...
# =================================================================
# PART 2: GENERATIVE MODEL CODE (from generate_samples.py)
# =================================================================
...
def run_pls_analysis(X_tr: np.ndarray, y_tr: np.ndarray,
    ...
    """
    Tunes, trains, and evaluates the PLS-DA model for a given dataset.
    """
    print(f"  Tuning PLS-DA (N={n_total_train})...")
    best_lv, cv_auc = tune_pls_da_auc(X_tr, y_tr, k=10, max_lv=30)
    ...
    if cm.size == 4:
        ...
        sens = tp / (tp + fn) if (tp + fn) else 0.0  # cancer recall
        spec = tn / (tn + fp) if (tn + fp) else 0.0  # healthy specificity
    else:  # Handle cases where one class is not predicted
        tn, fp, fn, tp = 0, 0, 0, 0
        sens, spec = np.nan, np.nan
        if np.all(y_te == yhat_te):  # 100% acc
            if np.all(y_te == 0): tn = len(y_te); spec = 1.0; sens = np.nan
            if np.all(y_te == 1): tp = len(y_te); sens = 1.0; spec = np.nan
        else:  # Find the single value
    ...
    return {
        ...
        "n_synthetic": n_total_train - len(y_te),  # Quick hack, assumes y_te length is same as y_tr_orig
        "best_lv": best_lv,
        "cv_auc": cv_auc,
        "test_auc": auc,
        "test_acc": acc,
...
def cm_as_matrix(y_true, y_pred):
    # Helper for edge cases in confusion_matrix
...
def main():
    ...
    if not (TRAIN_CSV.exists() and TEST_CSV.exists() and AE_META_FILE.exists()):
        print(f"Error: Missing one or more input files.")
        print(f"  {TRAIN_CSV} exists: {TRAIN_CSV.exists()}")
        print(f"  {TEST_CSV} exists: {TEST_CSV.exists()}")
        print(f"  {AE_META_FILE} exists: {AE_META_FILE.exists()}")
        return

    df_tr_orig = pd.read_csv(TRAIN_CSV)
    print(f"original train data.")
    print(df_tr_orig.head())
    ...
    # --- 2. Preprocess Test Data (Done once) ---
    print("Preprocessing test data...")
    X_te_preproc = preprocess_numpy_array(X_te_raw)

    # --- 3. Load Generative Models (Done once) ---
    ...
    x_mean = torch.from_numpy(np.array(gen_meta["scaler_mean"], dtype=np.float32)).to(DEVICE).view(1, 1, -1)
    x_std = torch.from_numpy(np.array(gen_meta["scaler_std"], dtype=np.float32)).to(DEVICE).view(1, 1, -1)
    ...
    for ratio in AUGMENT_RATIOS:
        ...
        if ratio == 0.0:
            ...
            print(f"  Generating {n_healthy_gen} healthy and {n_cancer_gen} cancer samples...")

            # Generate healthy
            x_gen_healthy = p_sample_loop(
                unet=unet, ae=ae, T=T_trained, steps=min(SAMPLE_STEPS, T_trained),
                y_class=0, n=n_healthy_gen, latent_c=latent_c, latent_L=latent_L,
            ...
            x_gen_cancer = p_sample_loop(
                unet=unet, ae=ae, T=T_trained, steps=min(SAMPLE_STEPS, T_trained),
                y_class=1, n=n_cancer_gen, latent_c=latent_c, latent_L=latent_L,
                x_mean=x_mean, x_std=x_std, z_mu=z_mu, z_std=z_std
            )
            y_gen_cancer = np.ones(n_cancer_gen, dtype=int)

            print(f"  Generated {len(x_gen_healthy)} + {len(x_gen_cancer)} new samples.")

            # Combine with original data
            X_tr_aug_raw = np.vstack([X_tr_orig_raw, x_gen_healthy, x_gen_cancer])
            y_tr_aug = np.hstack([y_tr_orig, y_gen_healthy, y_gen_cancer])
            print(f"  New training set size: {len(y_tr_aug)} (Original: {len(y_tr_orig)})")

        # --- Preprocess and Train ---
        print("  Preprocessing augmented training data...")
        X_tr_aug_preproc = preprocess_numpy_array(X_tr_aug_raw)

        # Run analysis (fix for n_synthetic count)
        n_syn = len(y_tr_aug) - len(y_tr_orig)
        results = run_pls_analysis(X_tr_aug_preproc, y_tr_aug, X_te_preproc, y_te, len(y_tr_aug), ratio)
        results["n_synthetic"] = n_syn  # Correctly set synthetic count
    ...
    # Save results to CSV
    results_csv_path = OUT_DIR / "augmentation_results.csv"
    df_results.to_csv(results_csv_path)
    print(f"Saved results table to: {results_csv_path}")

    # Print formatted table to console
    print("\nTest Set Performance vs. Augmentation Ratio:")
    print(df_results[
              ['n_total_train', 'n_synthetic', 'best_lv', 'test_auc', 'test_acc', 'test_sens', 'test_spec']].to_string(
        float_format="%.4f"))

    # --- Plot key metrics ---
    fig, axes = plt.subplots(nrows=2, ncols=2, figsize=(14, 10))
    fig.suptitle("PLS-DA Test Performance vs. Data Augmentation Ratio", fontsize=16)
    ...
    for ax in axes.flat:
        ax.set_ylim(bottom=max(0.0, df_results[['test_auc', 'test_acc', 'test_sens', 'test_spec']].min().min() - 0.1),
                    top=1.0)  # Set 1.0 as max for metrics

    plt.tight_layout(rect=[0, 0.03, 1, 0.95])

    plot_path = OUT_DIR / "augmentation_metrics_plot.png"
